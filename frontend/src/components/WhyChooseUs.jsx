  ...
  const features = [
    {
      ...
      desc: "Connect with verified sellers across Sri Lanka offering authentic spare parts. Every listing is reviewed for quality and accuracy.",
      icon: <Search className="w-8 h-8 text-red-600" />
    },
    {
      title: "Direct Deals",
      desc: "No middlemen. Negotiate and finalize transactions directly with the seller. Save money and build lasting supplier relationships.",
      icon: <Handshake className="w-8 h-8 text-red-600" />
    },
    {
      title: "Verified Sellers",
      desc: "We prioritize trust by highlighting reputable sellers within our community. PRO sellers go through a strict verification process.",
      icon: <ShieldCheck className="w-8 h-8 text-red-600" />
    }
  ];

  return (
    <div className="w-full max-w-7xl mx-auto px-6 py-24 flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
      ...
      <div className="lg:w-1/2 w-full">
        ...
        <h2 className="text-6xl md:text-7xl font-black text-white leading-[0.9] uppercase tracking-tighter mb-6">
          WHY TRUST <br />
      ...
      <div className="lg:w-1/2 w-full grid grid-cols-1 gap-4">
        {features.map((item, index) => (
          <div
            key={index}
            className="group bg-zinc-900/50 border border-zinc-800 p-10 hover:border-red-600 hover:bg-zinc-900 transition-all duration-500 flex items-start gap-8"
          >
